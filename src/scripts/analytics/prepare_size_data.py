...
if __name__ == "__main__":
    ...
    for c0, d in enumerate(tqdm(args.dirs)):
        ...
        stats = pd.read_csv(f'{d}/stats.csv')

        for c, f in enumerate(tqdm(os.listdir(d))):
            ...
            with open(f'{d}/{f}') as fi:
                size = len(''.join(fi.readlines()))

            k1_failed = not os.path.exists(f'{d}/v1/{fname}.jar')
            k2_failed = not os.path.exists(f'{d}/v2/{fname}.jar')
            
            failure_type = 0
            
            if k1_failed and k2_failed:
                if contains_error_of_type(f'{d}/v1/{fname}.txt', 'CONFLICTING_OVERLOADS') or contains_error_of_type(f'{d}/v1/{fname}.txt', 'UNRESOLVED_REFERENCE'):
                    failure_type = -1
                else:
                    failure_type = 5
